<table class="table table-bordered table-striped mb-0 p-0" id="absenMasuk" width="100%" style="font-size: 11px">
    <thead>
        <tr class="table-success">
            ...
            <th width="8%">Status/ <br>Validasi</th>
            <th width="16%" class="text-center">Ubah Status</th>
        </tr>
        <tr class="bg-yellow" style="font-size: 14px">
            <th colspan="10">
                @if($status == 'asn')
                Rekap Unconfirmed Absensi Masuk ASN
                @else
                Rekap Absensi Unconfirmed Masuk Non-ASN
                @endif
            </th>
    ...
    <tbody>
        @foreach ($dataAbsensiM as $dm)
        <tr>
            <td>{{ $loop->iteration }}</td>
            <td>{{ $dm->nama }}</td>
            <td>{{ $dm->nip }}</td>
            <td>
                <span class="tanggal">{{ $dm->tanggal }}</span>-{{ $dm->bulan }}-{{ $dm->tahun }}
            </td>
            <td>{{ $dm->waktu}}</td>
            <td>{{ $dm->keterangan }}</td>
            <td class="minus">{{ $dm->pengurangan }}</td>
            <td>
                <img src="{{ asset('storage/'.$dm->foto) }}" width="75px"
                    onclick="gbabsen({{ $dm->nama }}|{{ $dm->foto }})">
            </td>
            <td>
                <img src="{{ asset('storage/'.$dm->foto_b) }}" width="75px"
                    onclick="gbabsen({{ $dm->nama }}|{{ $dm->foto_b }})">
            </td>
            <td class="status">
                {{ $dm->konfirmasi }}
            </td>
            <td>
                <span class="d-flex justify-content-around">
                    <button class="btn btn-sm btn-info btn_{{ $dm->id }}" onclick="konfirmasi({{ $dm->id }})"
                        title="confirm"><i class="fas fa-check-circle"></i></button>
                    <button class="btn btn-sm btn-danger btn_{{ $dm->id }}" onclick="reject({{ $dm->id }})"
                        title="reject"><i class="fas fa-times-circle"></i></button>
                    <button class="btn btn-sm btn-danger btn_{{ $dm->id }}" onclick="hapus({{ $dm->id }})"
                        title="delete" tanggal="{{ $dm->tanggal }}" bulan="{{ $dm->bulan }}" tahun="{{ $dm->tahun }}"
                        nip="{{ $dm->nip }}"><i class="far fa-trash-alt"></i></button>
                </span>
            </td>
        </tr>
        @endforeach
...
$('.hapus').click(function(){
    ...
    function reject(id){
        ...
        var pengurangan = tr.find('.minus');
        $.get( "/absenMasuk/reject", { idtol: id } )
            .done(function( data ) {
                pengurangan.html(2);
